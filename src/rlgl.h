...
#ifndef RLGL_ASSERT
#include <assert.h>
#define RLGL_ASSERT assert
#endif

static rlglData* currentContext = 0;

void rlglSetContext(void *p)
{
    currentContext = p;
}

void *rlglCreateContext()
{
    void *p = malloc(sizeof(rlglData));
    memset(p, 0, sizeof(rlglData));
    return p;
}

void rlglDeleteContext(void *p)
{
    if (currentContext == p) {
        currentContext = 0;
    }
